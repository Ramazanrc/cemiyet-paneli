...
SUPABASE_KEY = "sb_publishable_rybhG-4_LuKfX7ceFBSd-A_iA2_Tajk"
...
<style>
    ...
    [data-testid="stAppViewContainer"] .stButton > button[kind="secondary"] {
        border-radius: 10px; transition: 0.2s; font-weight: 800;
    ...
    [data-testid="stAppViewContainer"] .stButton > button[kind="secondary"]:hover {
        transform: scale(1.03); box-shadow: 0px 5px 12px rgba(46, 139, 87, 0.15); border-color: #2e8b57 !important;
    }
    ...
    [data-testid="stAppViewContainer"] .stButton > button[kind="primary"] {
        ...
        height: auto !important; 
        line-height: 1.3 !important;
        box-shadow: 0 4px 10px rgba(46,139,87,0.3) !important;
    }
    [data-testid="stAppViewContainer"] .stButton > button[kind="primary"]:hover {
        transform: scale(1.03); box-shadow: 0px 6px 15px rgba(46, 139, 87, 0.4) !important;
    }
    ...
    /* SOL MENÜ (SIDEBAR) BUTONLARI */
    [data-testid="stSidebar"] .stButton>button[kind="secondary"] {
        border-radius: 12px; transition: 0.3s; font-weight: 800; font-size: 16px !important;
    ...
    [data-testid="stSidebar"] .stButton>button[kind="secondary"]:hover {
        background-color: #d1e8da !important; transform: translateX(6px); color: #1b5e20 !important;
    }
    ...
    [data-testid="stSidebar"] .stButton>button[kind="primary"]:hover { transform: translateX(0px) scale(1.02) !important; }
...
if 'zikirmatik_tur' not in st.session_state: st.session_state['zikirmatik_tur'] = 0 # Yeni Tur Sayacı
...
# --- ZİKİRMATİK MODÜLÜ (Tur Sayacına Sahip Yeni Tasarım) ---
def zikirmatik_bileseni(anahtar="ana"):
    st.markdown("<div class='bilgi-kutusu'><h3 style='color:#1e7145; margin-top:0px;'>🔢 Zikirmatik</h3><p style='color:gray; font-size:14px; margin-bottom:0px;'>Kendi hedefinizi yazın. Hedefe ulaştığınızda sayaç sıfırlanır ve alt kısımda attığınız tur sayısı belirtilir.</p></div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("🔄 Sayacı Sıfırla", key=f"z_sifirla_{anahtar}", use_container_width=True): 
            ...
            st.session_state['zikirmatik_tur'] = 0 # Turu da sıfırlar
            st.rerun()
    ...
    <style>
        div.element-container:has(.z-anchor-{anahtar}) + div.element-container button {{
            ...
            background: linear-gradient(to top, #a3e4d7 {yuzde}%, #ffffff {yuzde}%) !important; 
            border: 8px solid #ffffff !important; 
            outline: 5px dashed #45b39d !important;
            outline-offset: 4px !important;
            box-shadow: 0 20px 40px rgba(46,139,87,0.35), inset 0 10px 20px rgba(0,0,0,0.08) !important; 
            transition: transform 0.1s, background 0.3s ease !important; 
        ...
        div.element-container:has(.z-anchor-{anahtar}) + div.element-container button:active {{ transform: scale(0.92) !important; }}
    ...
    with col_center:
        ...
        # Eğer en az 1 tur atıldıysa alt satırda yıldızla tur sayısını göster
        ...
        if st.button(btn_icerik, key=f"z_btn_{anahtar}", use_container_width=True):
            ...
            if st.session_state['zikirmatik_sayac'] >= hedef:
                st.toast(f"Tebrikler! {hedef} hedefine ulaştınız. Sayacınız yeni tur için sıfırlandı.", icon="🎉")
                st.session_state['zikirmatik_sayac'] = 0
                st.session_state['zikirmatik_tur'] += 1 # Turu 1 artır
            st.rerun()

# --- DİNAMİK HATİM MOTORU ---
def dinamik_hatim_olusturucu(modul_baslik, ikon, state_key, ornek_hedef):
    st.markdown(f"<div class='bilgi-kutusu'><h3 style='color:#1e7145; margin-top:0px;'>{ikon} {modul_baslik}</h3><p style='color:gray; font-size:14px; margin-bottom:0px;'>Hedef belirleyebilir, zaman çizelgesi atayabilir ve manuel pay alabilirsiniz.</p></div>", unsafe_allow_html=True)
    ...
    aktifler = [h for h in st.session_state[state_key] if h['aktif']]
    if not aktifler: st.info("Devam eden kayıt bulunmuyor. Yeni bir tane başlatabilirsiniz.")
    ...
    for idx, h in enumerate(st.session_state[state_key]):
        if h['aktif']:
            ...
            with ust_c2:
                st.markdown(f"<div style='text-align:right; font-size:22px; font-weight:800; color:#45b39d;'>%{min(yuzde, 100)}</div>", unsafe_allow_html=True)
...
if not st.session_state['giris_yapildi']:
    ...
    sifre = st.text_input("Şifre:", type="password", label_visibility="collapsed", placeholder="Şifre (1453)")
...
# --- 2. ADIM: İSİM VE BÖLGE GİRİŞİ ---
    ...
    st.markdown("<p style='text-align: center; color: #5a7d66;'>Lütfen bilgilerinizi giriniz.</p>", unsafe_allow_html=True)
    
    isim = st.text_input("Ad Soyad:", placeholder="Örn: Ramazan Çeliktepe")
    bolge = st.text_input("Bölge / Kurum (İsteğe Bağlı):", placeholder="Örn: İstanbul / Fatih")
...
# --- 3. ADIM: ANA PANEL ---
    ...
    if menu == "📖 Kuran-ı Kerim Hatmi":
        st.markdown(f"<div class='bilgi-kutusu'><h3 style='color: #1e7145; font-weight: 800; margin-top:0px;'>📖 Ana Kuran Hatmi (Veritabanı)</h3><p style='margin-bottom:0px; color: #555;'>Merkezi 30 Cüz panosu. Lütfen boş cüzleri seçiniz.</p></div>", unsafe_allow_html=True)
        ...
        kalan_cuz = toplam_cuz - alinan_cuz
        ...
        col3.metric("Kalan", str(kalan_cuz))
        st.progress(yuzde)
        ...
        for cuz in cuzler:
            ...
            with sutunlar[(i-1) % 5]:
                if durum == "dolu": 
                    # Sayılar görünür hale getirildi "Cüz 1", "Cüz 2" şeklinde formatlandı
        ...
        with st.expander("➕ Yeni Kuran Hatmi Başlat"):
            c1, c2 = st.columns(2)
            y_k_ad = c1.text_input("Grup / Hatim Adı", placeholder="Örn: Hafta Sonu Grubu")
        ...
        for ek in st.session_state['ekstra_kuranlar']:
            if ek['aktif']:
                ...
                for c in range(1, 31):
                    ...
                    with ek_cols[(c-1) % 5]:
                        if alan != "": 
                            # Sayılar görünür hale getirildi
    ...
    # --- ZİKİR HATMİ (Zikirmatik Eklendi) ---
    ...
    # --- ÇETELE ---
        ...
        with col_tarih2:
            tarih_araligi = st.date_input("Zaman Çizelgesi Seç", value=(st.session_state['cetele_bas'], st.session_state['cetele_bitis']), label_visibility="collapsed")
            if len(tarih_araligi) == 2:
                if st.session_state['cetele_bas'] != tarih_araligi[0] or st.session_state['cetele_bitis'] != tarih_araligi[1]:
                    st.session_state['cetele_bas'] = tarih_araligi[0]; st.session_state['cetele_bitis'] = tarih_araligi[1]; st.rerun()
        ...
        if st.session_state['cetele_modu'] == 'ozet':
            ...
            with col2:
                ...
                if st.button(f"📝 Düzenle {tamamlanan}/{len(st.session_state['cetele'])}", use_container_width=True): st.session_state['cetele_modu'] = 'detay'; st.rerun()
            ...
            for baslik, veriler in st.session_state['cetele'].items():
                ...
                lc1, lc2, lc3 = st.columns([3, 4, 1.5])
                with lc1: st.markdown(f"<div style='font-weight:700; color:#333; margin-top:5px;'>{veriler['icon']} {baslik}</div>", unsafe_allow_html=True)
                with lc2: st.progress(min(yuzde, 100))
                with lc3: st.markdown(f"<div style='background-color:#eaf3ed; border-radius:10px; padding:3px 8px; text-align:center; font-weight:800; color:#45b39d;'>{veriler['okunan']}/{veriler['hedef']}</div>", unsafe_allow_html=True)
            ...
            for baslik, veriler in st.session_state['cetele'].items():
                ...
                with uc2:
                    if veriler['okunan'] >= veriler['hedef']: st.button("✅ Tamamlandı", key=f"f_{baslik}", disabled=True, type="primary", use_container_width=True)
                        ...
                        if st.button("✔️ Tamamla", key=f"d_{baslik}", type="primary", use_container_width=True): st.session_state['cetele'][baslik]['okunan'] = veriler['hedef']; st.rerun()
                ...
                with ac4:
                    st.markdown("<div style='font-size:12px; font-weight:bold; color:#1e7145;'>Hedef:</div>", unsafe_allow_html=True)
    ...
    # --- ARŞİV ---
        ...
        st.markdown(f"<div class='bilgi-kutusu'><h3 style='color: #1e7145; font-weight: 800; margin-top:0px;'>🗂️ Tamamlanan Hatimler Arşivi</h3><p style='color:gray; margin-bottom:0px;'>Hedefine ulaşarak arşive kaldırılmış geçmiş programlar burada listelenir.</p></div>", unsafe_allow_html=True)
        arsiv_dolu = False
        ...
        for isim, state_key in kategoriler.items():
            bitenler = [h for h in st.session_state[state_key] if not h['aktif']]
            if bitenler:
                arsiv_dolu = True
    ...
    # --- MODERATÖR PANELİ ---
        ...
        if not st.session_state['admin_yetkisi']:
            admin_sifre = st.text_input("Yönetici Şifresi:", type="password", placeholder="Şifre: patron123")
            ...
            for cuz in response.data:
                ...
                if cuz["durum"] == "dolu" and kisi:
                    if kisi not in kullanici_ozetleri:
                        kullanici_ozetleri[kisi] = {"Kuran": [], "Yasin": "-", "İhlas": "-", "Salavat": "-", "Zikir": "-", "Cevşen": "-"}
                    kullanici_ozetleri[kisi]["Kuran"].append(str(cuz["cuz_no"]))
            
            if st.session_state['kullanici_adi'] in kullanici_ozetleri:
                kullanici_ozetleri[st.session_state['kullanici_adi']]["Yasin"] = "1 Adet"
                kullanici_ozetleri[st.session_state['kullanici_adi']]["İhlas"] = "1.000"
                kullanici_ozetleri[st.session_state['kullanici_adi']]["Salavat"] = "500"
                kullanici_ozetleri[st.session_state['kullanici_adi']]["Zikir"] = "1.000"
                kullanici_ozetleri[st.session_state['kullanici_adi']]["Cevşen"] = "Duâ-yı Cevşenü'l-Kebîr"

            tablo_verisi = []
            for kisi, veriler in kullanici_ozetleri.items():
                tablo_verisi.append({
                    "👤 İsim": kisi,
                    "📖 Kuran": ", ".join(veriler["Kuran"]),
                    "🕌 Yasin": veriler["Yasin"],
                    "🕋 İhlas": veriler["İhlas"],
                    "📿 Salavat": veriler["Salavat"],
                    "🤲 Zikir": veriler["Zikir"],
                    "🛡️ Cevşen": veriler["Cevşen"]
                })

            st.markdown("*Kuran Cüzleri canlı veridir. Diğer sütunlar sistem bağlandığında dolacaktır.*")
            if tablo_verisi: st.dataframe(tablo_verisi, use_container_width=True, hide_index=True)
            else: st.info("Henüz kimse cüz almadı.")
            
            if st.button("🔄 Ana Kuran Cüzlerini Sıfırla", use_container_width=True):
    ...
    if st.sidebar.button("🚪 Güvenli Çıkış Yap", use_container_width=True):
        ...
        st.session_state['bolge'] = ""
        st.session_state['admin_yetkisi'] = False 
        st.rerun()